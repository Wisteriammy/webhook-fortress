...
class WebhookFortress:
    ...
    def init_db(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS webhooks (
                ...
                proxy_path TEXT UNIQUE NOT NULL,
    ...
    def create_proxy_link(self, original_url, expires_in_hours=72):
        ...
        cursor = self.db.cursor()
        cursor.execute('''
            INSERT INTO webhooks (id, original_url, proxy_path, expires_at)
            VALUES (?, ?, ?, ?)
        ''', (webhook_id, original_url, proxy_path, expires_at))
        self.db.commit()

        proxy_link = f"http://localhost:5000/proxy/{proxy_path}"

        with self.lock:
            self.proxy_links[proxy_path] = {
    ...
    def verify_proxy_path(self, proxy_path):
        ...
        cursor.execute('''
            ...
            FROM webhooks WHERE proxy_path = ?
        ''', (proxy_path,))
    ...
    def log_request(self, webhook_id, ip):
        cursor = self.db.cursor()
        cursor.execute('''
            INSERT INTO requests (webhook_id, ip_address)
            VALUES (?, ?)
        ''', (webhook_id, ip))
        self.db.commit()
...
def proxy_webhook(webhook_data, proxy_path):
    try:
        ...
        headers = {
            ...
            'User-Agent': f'WebhookFortress/{BRAND_NAME}'
        }

        response = requests.post(
            webhook_data['original_url'],
            json=payload,
            headers=headers,
            timeout=10
        )
...
def list_webhooks():
    try:
        cursor = fortress.db.cursor()
        cursor.execute('''
            SELECT id, original_url, proxy_path, created_at, expires_at, is_active
        ...
        for row in results:
            webhooks.append({
                'id': row[0],
                'original_url': row[1][:50] + '...' if len(row[1]) > 50 else row[1],
                'proxy_path': row[2],
                'created_at': row[3],
                'expires_at': row[4],
...
if __name__ == '__main__':
    print("=" * 50)
    print(f"🔒 WEBHOOK FORTRESS - {BRAND_HEADER}")
    print("=" * 50)
    print(f"📍 http://localhost:5000")
    print("📌 POST /create - Create protected webhook")
    print("📌 GET  /proxy/<path> - Shows brand message")
    print("📌 POST /proxy/<path> - Forward to webhook")
    print("=" * 50)
